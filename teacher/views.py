...
def rosterview(request,pk):
    ...
    return render(request,'teacher/rosterview.html',{'student_classes':student_classes,'my_class':my_class,'nbar':'teacher'})
...
def studentoneclassview(request,**kwargs):
    ...
    return render(request,'teacher/studentoneclassview.html',context)
...
def studentclassview(request,**kwargs):
    ...
    context['username'] = student_username
    return render(request,'teacher/studentview.html',context)
...
def load_grade(request,**kwargs):
    ...
    for i in range(0,po.point_value+1):
        pts.append(i)
    context['pts'] = pts
    return JsonResponse({'modal-html':render_to_string('teacher/modal-edit-grade.html',context)})
...
def slidesview(request,**kwargs):
    ...
    return render(request,'teacher/slidesview.html',{'slides':slidegroup,'rows':rows,'class':pub_class})
...
def getstudentlist(request,pk):
    ...
    return JsonResponse({'students':render_to_string('teacher/roster-studentselect.html',{'students':students})})
...
def classeditview(request,pk):
    ...
    context['nbar'] = 'teacher'
    return render(request, 'teacher/editclassview.html',context)
...
def newunitview(request,pk):
    ...
    if request.method == "POST":
        ...
        my_class.units.add(u)
        my_class.save()
        return HttpResponse(render_to_string('teacher/unitsnippet.html',{'unit':u,'forcount':my_class.units.count()}))
    return HttpResponse('')
...
def newslidesview(request,pk,upk):
    ...
    if request.method == "POST":
        ...
        s=SlideGroup(name=form.get("slides-name",""),unit_object = u)
        s.save()
        return HttpResponse(render_to_string('teacher/unitobjectsnippet.html',{'unitobj':u,'forcount':unit.unit_objects.count()}))
    return HttpResponse('')
...
def problemseteditview(request,pk,upk,ppk):
    ...
    context['nbar'] = 'teacher'
    return render(request, 'teacher/editproblemsetview.html',context)
    ...
    if request.method == "POST":
        ...
        if qt == "multiple choice":
            ...
            if pform.is_valid():
                ...
                problemset.problem_objects.add(prob)
                problemset.save()
                return JsonResponse({'problem_text':render_to_string('teacher/problemobjectsnippet.html',{'probobj':prob,'forcount':problemset.problem_objects.count()}),'pk':prob.pk})
        elif qt == "short answer":
            pform=NewProblemObjectSAForm(request.POST, instance=po)
            if pform.is_valid():
                prob=pform.save()
    ...
    if request.method == "POST":
        ...
        if 'save' in form:#######
            ...
            for i in range(0,len(slide_inputs)):
                ...
                s.order = i+1
                s.save()
            return JsonResponse({'slidelist':render_to_string('teacher/slidelist.html',{'slides' :slidegroup})})
    context={}
    context['my_class'] = my_class
    context['unit'] = unit
    context['slides'] = slidegroup
    context['nbar'] = 'teacher'
    return render(request,'teacher/editslidesview.html',context)
...
def newslideview(request,pk,upk,spk):
    ...
    if request.method == "POST":
        ...
        s.save()
        return JsonResponse({'slide-body':render_to_string('teacher/slidebody.html',{'slide':s,'forcount':s.order})})
#    return HttpResponse('')
...
def editslideview(request,pk,upk,spk,sspk):
    ...
    if request.method == "POST":
        ...
        if "addtextblock" in form:
            ...
            slide.save()
            return JsonResponse({'textblock':render_to_string('teacher/slideobject.html',{'s':s}),'sopk':s.pk})
        if "addtheorem" in form:
            ...
            slide.top_order_number = slide.top_order_number +1
            slide.save()
            return JsonResponse({'theorem':render_to_string('teacher/slideobject.html',{'s':s}),'sopk':s.pk})
        if "addproof" in form:
        ...
        if "addimage" in form:
            ...
            if form.is_valid():
                ...
                slide.top_order_number = slide.top_order_number +1
                slide.save()
                return JsonResponse({'image':render_to_string('teacher/slideobject.html',{'s':s}),'sopk':s.pk})
            ...
            if source == "bylabel":
                ...
                if Problem.objects.filter(label = problem_label).exists():
                    ...
                    if p.type_new in userprofile.user_type_new.allowed_types.all():
                        ...
                        slide.top_order_number = slide.top_order_number + 1
                        slide.save()
                        return JsonResponse({'example':render_to_string('teacher/slideobject.html',{'s':s}),'sopk':s.pk})
                    else:
...
#                pass
        ...
        if 'save' in form:
            ...
            slide.save()
            return JsonResponse({'slideobjectlist':render_to_string('teacher/slideobjectlist.html',{'slide_objects' :slide.slide_objects.all()})})
    context={}
    context['my_class'] = my_class
    context['unit'] = unit
    context['slides'] = slidegroup
    context['slide'] = slide
    context['nbar'] = 'teacher'
    return render(request,'teacher/edit-slide.html',context)

@login_required
def exampleoriginalqt(request,**kwargs):
    return JsonResponse({'qt-form':render_to_string('teacher/example-original-question-type.html')})

@login_required
def exampleproblemlabel(request,**kwargs):
    return JsonResponse({'pl-form':render_to_string('teacher/example-problem-label.html')})

@login_required
def exampleproblemgroups(request,**kwargs):
    return JsonResponse({'pg-form':render_to_string('teacher/example-problem-group.html',{'pg':request.user.userprofile.problem_groups.all()})})
...
def exampleproblemgroupproblems(request,**kwargs):
    ...
    return JsonResponse({'pgp-form':render_to_string('teacher/example-problem-group-problems.html',{'problem_group':problem_group})})
...
def examplebytag(request,**kwargs):
    ...
    return JsonResponse({'tag-form':render_to_string('teacher/example-bytag.html',{'contest_types':contest_types,'tags':tags})})
...
def examplebytagproblems(request,**kwargs):
    ...
    return JsonResponse({'problems':render_to_string('teacher/example-bytag-problems.html',{'rows':problems})})
...
def editexampleproblem(request,pk,upk,spk,sspk,sopk):
        ...
        if ep.isProblem == 0:
            if qt == "multiple choice":
                ...
                if pform.is_valid():
                    ...
                    prob.author = request.user
                    prob.save()
                    return JsonResponse({'prob':render_to_string('teacher/slideobject.html',{'s':so}),'qt':qt,'sopk':so.pk})
            elif qt == "short answer":
                pform=NewExampleProblemSAForm(request.POST, instance=ep)
                if pform.is_valid():
                    prob=pform.save()
    ...
    if qt == 'proof':
        form = NewExampleProblemPFForm(instance=ep)
    return JsonResponse({'form':render_to_string('teacher/originalproblemform.html',{'form':form}),'qt':qt})

class TextBlockUpdateView(UpdateView):
    model = TextBlock
    form_class = TextBlockForm
    template_name = 'teacher/textblock_edit_form.html'
...
class TheoremUpdateView(UpdateView):
    ...
    template_name = 'teacher/theorem_edit_form.html'
...
class ProofUpdateView(UpdateView):
    ...
    template_name = 'teacher/proof_edit_form.html'
...
def currentclassview(request,pk):#only for published classes...grading; viewing; etc.
    userprofile=request.user.userprofile
    my_class = get_object_or_404(PublishedClass,pk=pk)
    return render(request,'teacher/classview.html',{'class':my_class})

@login_required
def studentmanager(request):#request student accounts; add to classes
    userprofile = request.user.userprofile
    return render(request,'teacher/studentmanager.html',{'userprofile':userprofile})
...
def blindgrade(request,**kwargs):
    ...
    shuffle(responses)
    return render(request,'teacher/blindgrading.html',{'problem_object':problem_object,'responses':responses,'class':my_class,'problemset':problemset})
...
def alphagrade(request,**kwargs):
    ...
    return render(request,'teacher/alphagrading.html',{'problem_object':problem_object,'responses':responses,'class':my_class,'problemset':problemset})
...
def grouptableview(request):
    ...
    template=loader.get_template('teacher/grouptableview.html')
...
#            print(form)
    ...
    template=loader.get_template('teacher/probgroupview.html')
    return HttpResponse(template.render(context,request))
...
def fetchproblems(request,pk):
    ...
    for i in range(0,len(prob_list)):
        ...
        prob_code.append(render_to_string('teacher/problemsnippet2.html',{'prob':prob_list[i],'forcount':base_num+i+1}))
    prob_group.save()
    data = {
                'prob_list': prob_code,
            }
