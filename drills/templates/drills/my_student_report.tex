...
\begin{center}
    ...
    \begin{longtable}{c|{% for i in problem_numbers %}c{% endfor %}|c}
        Drill&{% for i in problem_numbers %}{{forloop.counter}}&{%endfor%}Total Score\\\hline
        {% for drill_record,buffer in drill_records %}
        {{drill_record.drill.readable_label|poundsafe}}{% for i in drill_record.drill_record_problems.all %}&{% if i.status != -1 %}{{i.status}}{% else %}&{% endif %}{% endfor %}{% for i in buffer %}&{% endfor %}&{{drill_record.score}}\\
        {% endfor %}
    \end{longtable}
\end{center}
{% else %}
