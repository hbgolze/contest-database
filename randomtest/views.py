...
def tableview(request):
    ...
    atests=userprof.archived_tests.all()
...
#        testresponses=userprof.allresponses.filter(test=tests[i])
        if Responses.objects.filter(test=tests[i],user_profile=userprof).exists() == False:
            allresponses=Responses(test=tests[i],num_problems_correct=0)
            allresponses.save()
            P=tests[i].problems.all()
            for j in range(0,len(P.count())):
...
#        if testresponses.count()==0:
        if Responses.objects.filter(test=atests[i],user_profile=userprof).exists() == False:
            allresponses=Responses(test=atests[i],num_problems_correct=0)
            allresponses.save()
            P=atests[i].problems.all()
            for j in range(0,P.count()):
                r=Response(response='',problem_label=P[j].label)
                r.save()
                allresponses.responses.add(r)
