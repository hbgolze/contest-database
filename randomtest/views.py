...
def testview(request,**kwargs):#switching to UserTest
    ...
    if request.method == "POST" and 'username' not in kwargs:
        ...
        R=list(allresponses.responses.all())
...
#This still isn't adding to the Sticky queue...
@login_required
def toggle_star(request,pk):
    ...
    usertest=response.responses.usertest#fails because response does not have a foreignkey freference to responses...could probably get rid of responses altogether and just do a foreignkey reference to usertest
...
def checkanswer(request,pk):
    ...
    allresponses = usertest.responses
    r=allresponses.responses.get(problem_label=problem_label)
    ...
    if r.response != tempanswer:# i.e., new answer
        ...
        if prob.question_type_new.question_type == "multiple choice":
            if r.response==prob.mc_answer and prob.question_type_new.question_type !='proof':
                ...
                ur.save()
            userprofile.responselog.add(ur)
            allresponses.num_problems_correct = allresponses.num_problems_correct + 1
            allresponses.show_answer_marks=1
            allresponses.save()
            usertest.num_correct = allresponses.num_problems_correct
            usertest.save()
        elif prob.question_type_new.question_type == "short_answer":
