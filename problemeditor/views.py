...
def view_entry_log(request):
    pk = request.GET.get('pk')
    prob = get_object_or_404(Problem,pk = pk)
    prob_entries = list(LogEntry.objects.filter(object_id = pk).filter(content_type_id = ContentType.objects.get_for_model(prob).pk))
    sol_entries = []
    for sol in prob.solutions.all():
        sol_entries += list(LogEntry.objects.filter(object_id = sol.pk).filter(content_type_id = ContentType.objects.get_for_model(sol).pk))
